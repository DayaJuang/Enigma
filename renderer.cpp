...
static ID3D11Buffer*			g_NormalMapBuffer = NULL;
...
static NORMALMAPPING	g_NormalMap;
...
{
	g_NormalMap.normalMap = flag;
	GetDeviceContext()->UpdateSubresource(g_NormalMapBuffer, 0, NULL, &g_NormalMap, 0, 0);
}
...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	hr = D3DX11CompileFromFile("vs_Simple.hlsl", NULL, NULL, "VertexShaderPolygon", "vs_4_0", shFlag, 0, NULL, &pVSBlob, &pErrorBlob, NULL);
	if (FAILED(hr))
	{
	...
	hr = D3DX11CompileFromFile("ps_Simple.hlsl", NULL, NULL, "PixelShaderPolygon", "ps_4_0", shFlag, 0, NULL, &pPSBlob, &pErrorBlob, NULL);
	if (FAILED(hr))
	{
	...
	//バンプマップ
	hBufferDesc.ByteWidth = sizeof(NORMALMAPPING);
	g_D3DDevice->CreateBuffer(&hBufferDesc, NULL, &g_NormalMapBuffer);
	g_ImmediateContext->VSSetConstantBuffers(8, 1, &g_NormalMapBuffer);
	g_ImmediateContext->PSSetConstantBuffers(8, 1, &g_NormalMapBuffer);
